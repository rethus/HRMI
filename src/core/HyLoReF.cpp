...
void HyLoReF(double *removedData, double *testData, double *weightData, double *weightData2, int numUser, int numService, int dim, double gamma,
    ...
    double **weightMatrix2 = vector2Matrix(weightData2, numService, numService);
    ...
    unordered_map<int, vector<int>> serviceNeighbors = findNeighbors(weightMatrix2, numService);
    ...
    for (iter = 0; iter < maxIter; iter++) {
        ...
        // update gradients
        gradLoss(U, S, removedMatrix, weightMatrix, weightMatrix2, userNeighbors, serviceNeighbors, gradU, gradS, gamma, lmda, eta, eta2, theta, numUser, numService, dim);
        ...
        if (debugMode) {
            // update loss value
            lossData[iter] = loss(U, S, removedMatrix, weightMatrix, weightMatrix2, userNeighbors, serviceNeighbors, gamma, lmda, eta, eta2, theta, numUser, numService, dim);
        }
...
double loss(double **U, double **S, double **removedMatrix, double **weightMatrix, double **weightMatrix2,
    ...
    double neighbor_k2;
    for (j = 0; j < numService; j++) {
        vector<int> neighbors = serviceNeighbors[j];
        for (k = 0; k < dim; k++) {
            neighbor_k2 = 0;
            for (int ne: neighbors) {
                neighbor_k2 += S[ne][k] * weightMatrix2[j][ne];
            }
            loss += 0.5 * eta2 * square(S[j][k] - neighbor_k2);
        }
    }
...
void gradLoss(double **U, double **S, double **removedMatrix, double **weightMatrix, double **weightMatrix2,
    ...
    for (j = 0; j < numService; j++) {
        ...
        for (k = 0; k < dim; k++) {
            ...
            neighbor_k = 0;
            for (int ne: neighbors) {
                neighbor_k += S[ne][k] * weightMatrix2[j][ne];
            }
            grad += eta2 * (S[j][k] - neighbor_k);
            gradS[j][k] = grad;
        }
    }
//    delete2DMatrix(predMatrix);
